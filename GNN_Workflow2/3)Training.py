    ...
    if os.path.exists(graphs_path):
        ...
        for frame_id in tqdm(frame_ids, desc="Processing frames"):
            ...
            if graph is not None:
                ...
                # tids = ak.to_numpy(frame_hits['tid_array'])[~np.isnan(ak.to_numpy(frame_hits['gx']))]  # align with valid_mask
                # layers = ak.to_numpy(frame_hits['layer_array'])[~np.isnan(ak.to_numpy(frame_hits['gx']))]  # align with valid_mask
                # evaluate_edge_coverage(graph, tids, layers)
