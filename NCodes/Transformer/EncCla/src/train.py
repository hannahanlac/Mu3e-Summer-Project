...
def train_epoch(
    ...
    for batch in tqdm(trainloader, desc=f"Epoch {epoch+1}", unit= "batch"):
        inputs,labels = batch
    ...
    print(f"Epoch {epoch+1} completed: Training loss = {epoch_loss:.4f}, Accuracy = {epoch_accuracy:.2f}%")
    wandb_logger.log(
...
def test(model, testloader, helperloader, truths_df, device, output_dir, wandb_logger):
    model.eval()
    ...
    with torch.no_grad():
        for (inputs, labels), (hit_ids, event_ids) in zip(testloader, helperloader):  # per batch
            ...
            outputs = model(inputs).view(-1, model.num_classes)
            labels = labels.view(-1)
            _, predicted = torch.max(outputs.data, 1) #New

            test_metrics_calculator.update(outputs, labels)
            test_metrics_calculator.add_true_score(
                hit_ids, event_ids, outputs, truths_df
            )
            # Collect predictions (flattened), hit_ids, and event_ids
            flat_hit_ids = hit_ids.view(-1).cpu().numpy()
            # Here, you might need to replicate event_ids to match the number of hits per frame, 
            # or if your helper collate function returns a 1D tensor per frame, use that directly.
            flat_event_ids = event_ids.view(-1).cpu().numpy()  
            flat_predictions = predicted.cpu().numpy()

            all_hitIndexes.extend(flat_hit_ids)
            all_frameNumbers.extend(flat_event_ids)     #new
            all_predictions.extend(flat_predictions)

    # Create a DataFrame of predictions
    predictions_df = pd.DataFrame({
        "frameNumber": all_frameNumbers,
        "hitIndex": all_hitIndexes,
        "predicted_bin_index": all_predictions   #new
    })
    ...
    # Save CSV for later evaluation or inspection
    csv_path = os.path.join(output_dir, "predictions_test_with_truths.csv")
    predictions_truth_merge.to_csv(csv_path, index=False)                   
...
def main(config_path):
    ...
    test(model, test_loader, helper_loader, truths_df, device, output_dir, wandb_logger)
