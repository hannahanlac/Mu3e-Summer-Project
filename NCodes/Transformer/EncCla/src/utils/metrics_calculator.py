...
class MetricsCalculator:
    ...
    def add_true_score(self, hit_ids, event_ids, outputs, truths_df):
        ...
        predictions_df = pd.DataFrame(
            {
                "hitIndex": hit_ids,
                "predicted_bin_index": predicted,  # Track IDs
                "frameNumber": event_ids,  # Event IDs (frame numbers, each frame is a separate 'event')
            }
        )

        unique_event_ids = np.unique(event_ids) #Should be 9921 really
        for event_id in unique_event_ids:
            if event_id == 0:
                continue  # Skip padding
            event_predictions_df = predictions_df[predictions_df["frameNumber"] == event_id]
            event_truths_df= truths_df.loc[truths_df["frameNumber"] == event_id].copy().assign(weight=1.0)

            # event_truths_df.loc[:,"weight"] = 1.0  # Assuming equal weight for all hits

            # Debugging prints
            print("Calculating scores")
            print(f"Event ID: {event_id}")
            #print("Event Predictions DF Columns:", event_predictions_df.columns)
            #print(event_predictions_df.head())  # Print first few rows
            #print("Event Truths DF Columns:", event_truths_df.columns)
            #print(event_truths_df.head())
