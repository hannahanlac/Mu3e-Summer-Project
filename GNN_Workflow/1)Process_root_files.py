...
def process_single_root_file(root_file):
    """Process a single ROOT file and return hit and truth data."""
    ...
    return hit_data, truth_data

def ProcessRootFiles(root_dir, output_dir):
    """Process all ROOT files in a given directory and generate training data (parallelized)."""
    ...
    # Parallel processing of ROOT files
    with concurrent.futures.ProcessPoolExecutor() as executor:
        results = list(executor.map(process_single_root_file, root_files))

    # Collect results and adjust frame numbers
    for hit_data, truth_data in results:
    ...
    merged_hits = pd.concat(all_hit_data, ignore_index=True)
    merged_truth = pd.concat(all_truth_data, ignore_index=True)

    print("Linking all hits and truth data:")
    merged_hits_truth = LinkHitsTruthData(merged_hits, merged_truth, signal_no="merged")

    merged_csv_path = os.path.join(output_dir, "new_two_processed_signals.csv")
...
root_dir = "/users/gy22186/mu3e/new_two_signal_files"
output_dir = "/users/gy22186/mu3e/new_two_signal_files"
