...
def batch_frames(awk_array, frames_per_batch=50):
    frame_ids = np.unique(ak.to_numpy(awk_array['frame_array']))
    batches = [frame_ids[i:i+frames_per_batch] for i in range(0, len(frame_ids), frames_per_batch)]
    return batches

def get_hits_for_frame(awk_array, frame_id):
    mask = ak.to_numpy(awk_array['frame_array']) == frame_id
    return awk_array[mask]
...
def edge_classifier(edge_features, channels=(32, 64), with_bn=True, activation='relu', name='edge_classifier'):
    ...
    Returns:
        edge_logits: (E, 1) - Probability of being the same track
    """
...
# Build a graph from the first frame to get feature shape
first_frame_id = batch_frames(awk_data, frames_per_batch=50)[0][0]
first_frame_hits = get_hits_for_frame(awk_data, first_frame_id)
G, truth_info = build_graph(first_frame_hits)
edge_feats, _, _ = extract_edge_features(G, truth_info)

# Initialize model with correct input shape
model = edge_classifier(np.zeros((1, edge_feats.shape[1])))
# Compile model
model.compile(optimizer='adam', loss='binary_crossentropy', metrics=['accuracy'])

frames_per_batch = 50  # You can tune this for your GPU
batches = batch_frames(awk_data, frames_per_batch)

num_epochs = 2
total_steps = num_epochs * len(batches)

def build_graph_for_frame(frame_id):
    frame_hits = get_hits_for_frame(awk_data, frame_id)
    return build_graph(frame_hits)

for epoch in range(num_epochs):
    print(f"\nEpoch {epoch+1}/{num_epochs}")

    # Print optimizer weights if they exist, otherwise print a message
    if hasattr(model.optimizer, 'variables'):
        print(f"Optimizer weights (start epoch {epoch+1}):", [v.shape for v in model.optimizer.variables])
    else:
        print(f"Optimizer has no variables at start of epoch {epoch+1}")
    epoch_losses = []
    epoch_accuracies = []
    epoch_graph_time = 0
    epoch_feat_time = 0
    ...
    for batch_idx, batch_frame_ids in enumerate(batches):
        batch_edge_feats = []
        batch_edge_lbls = []

        # Build graphs sequentially
        t_graph_start = time.time()
        results = [build_graph_for_frame(fid) for fid in batch_frame_ids]
        t_graph_end = time.time()
        epoch_graph_time += t_graph_end - t_graph_start

        # --- Feature extraction ---
        t_feat_start = time.time()
        for G, truth_info in results:
            edge_feats, edge_lbls, _ = extract_edge_features(G, truth_info)
            if edge_feats.shape[0] > 0:
                batch_edge_feats.append(edge_feats)
                batch_edge_lbls.append(edge_lbls)
        t_feat_end = time.time()
        epoch_feat_time += t_feat_end - t_feat_start

        if not batch_edge_feats:
            print(f"Skipping empty batch {batch_idx+1} at epoch {epoch+1}")
            continue

        # --- Training ---
        t_train_start = time.time()
        edge_feats_tensor = tf.convert_to_tensor(np.concatenate(batch_edge_feats), dtype=tf.float32)
        edge_lbls_tensor = tf.convert_to_tensor(np.concatenate(batch_edge_lbls), dtype=tf.float32)
        #Debugging: Print tensor shapes and model state
        print(f"Epoch {epoch+1}, Batch {batch_idx+1}: edge_feats_tensor shape: {edge_feats_tensor.shape}, edge_lbls_tensor shape: {edge_lbls_tensor.shape}")
        print(f"Model trainable: {model.trainable}")

        # Try/except to catch silent errors
        t_train_start = time.time()
        try:
            loss, acc = model.train_on_batch(edge_feats_tensor, edge_lbls_tensor)
        except Exception as e:
            print(f"Training error in epoch {epoch+1}, batch {batch_idx+1}: {e}")
            loss, acc = np.nan, np.nan
        t_train_end = time.time()
        epoch_train_time += t_train_end - t_train_start
    
        epoch_losses.append(loss)
        epoch_accuracies.append(acc)

        print(f"Batch {batch_idx+1}/{len(batches)}: "
              f"Graph {t_graph_end-t_graph_start:.2f}s, "
              f"Feat {t_feat_end-t_feat_start:.2f}s, "
              f"Train {t_train_end-t_train_start:.2f}s, "
              f"Loss {loss:.4f}, Acc {acc:.4f}")
    # Print optimizer weights at the end of the epoch
    if hasattr(model.optimizer, 'variables'):
        print(f"Optimizer weights (end epoch {epoch+1}):", [v.shape for v in model.optimizer.variables])
    else:
        print(f"Optimizer has no variables at end of epoch {epoch+1}")
    avg_loss = np.mean(epoch_losses)
    avg_acc = np.mean(epoch_accuracies)
    print(f"Epoch {epoch+1} summary: "
          f"Graph {epoch_graph_time:.2f}s, "
          f"Feat {epoch_feat_time:.2f}s, "
          f"Train {epoch_train_time:.2f}s, "
          f"Avg Loss {avg_loss:.4f}, Avg Acc {avg_acc:.4f}")

print("Training complete! Saving model...")
model.save("trained_gnn_model_gpu.keras")