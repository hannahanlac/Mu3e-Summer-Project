...
# Preprocess the 'tids' column to handle np.int64 objects
df['tids'] = df['tids'].apply(lambda x: eval(x, {"np": np}))
print("Inspecting 'tids' column after preprocessing:")
print(df['tids'].head(10))  # Print the first 10 rows of the 'tids' column
print(df['tids'].apply(type).value_counts())  # Check the types of the elements in the column
df['hit_IDs'] = df['hit_IDs'].apply(lambda x: eval(x, {"np": np}))
# Define the function to check the percentage of hits belonging to the same tid
def check_correct_tracks(tids, threshold, inclusive=False):
...
def update_tid_column(tids, threshold, inclusive=False):
    tid_counts = pd.Series(tids).value_counts()
    if inclusive:
...
merged_df['hit_IDs'] = merged_df['hit_IDs'].apply(safe_parse_hit_ids)

# Calculate num_hits_y as the number of hits in hit_IDs
merged_df['num_hits_y'] = merged_df['hit_IDs'].apply(lambda x: len(x) if isinstance(x, list) else 0)

# Debugging the preprocessed column
print("Inspecting 'hit_IDs' column after preprocessing:")
print(merged_df['hit_IDs'].head(10))
print(merged_df['hit_IDs'].apply(type).value_counts())
...
output_file_path = f'{output_directory}/predicted_tracksgine_merged_{safe_command}_harsh.csv'
